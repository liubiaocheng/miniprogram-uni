	...
	<view class="jsfun-record" @tap="showPicker">
		...
		<view class="conbox record" :class="{'pickerShow':isShow}">
			...
			<view class="record-box" @touchstart="start" @longpress="record" @touchend="end" @touchmove.stop.prevent="moveHandle">
				<span class="stop" @touchstart.stop="stopVoice" v-if="recordTime >= minTime && playing==1"></span>
				<span class="paly" @touchstart.stop="playVoice" v-if="recordTime >= minTime && playing==0"></span>
				<canvas class="canvas" canvas-id="canvas">
					<span class="recording"></span>
				</canvas>
				<span class="confirm" @touchstart.stop="okClick" v-if="recordTime >= minTime"></span>
			</view>
			<view class="c666 fz32 domess">{{bottomText}}</view>
		</view>
	</view>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			showPicker: function() {
				...
				uni.authorize({
					...
					fail: function() {
						uni.getSetting({
							success: (res) => {
								let authStatus = res.authSetting['scope.record'];
								if (!authStatus) {
									uni.showModal({
										title: '授权失败',
										content: '小程序需要获取录音权限，请在设置界面打开.',
										// showCancel:false,// todo 未授权应收回录音界面
										success: (res) => {
											if (res.confirm) {
												uni.openSetting()
											}
			...
			start: function() {
				...
				this.recordTime = 0;
				//生成canvas对象
				this.canvasObj = uni.createCanvasContext('canvas', this); //fixed
			},
			end: function() {
				...
				this.bottomText = "点击左侧试听, 长按中间重录, 点击右侧上传录音."
			},
			record: function() {
				let _this = this;
			...
			playVoice() {
				if (this.voicePath) {
					...
					innerAudioContext.loop = true;// 循环播放录音
...
<style lang="scss">
	.jsfun-record {
		...
		.record {
			...
			.stop {
				@extend .btncom; // Cannot read property 'extend' of undefined
...
<!-- https://ext.dcloud.net.cn/plugin?id=436 -->
